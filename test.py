...
parser.add_argument('--check_path', type=str, default='outputs/checkpoints/',
...
data_paths=glob(datapaths,'*',recursive=True)
ds=oralDataset_test(data_paths,tform_test)
testdataloader = DataLoader(ds, batch_size=batch_size, shuffle=True)

###### model
checkpahts=glob(checkpoint_path,'*',recursive=True)
model_base = EfficientNet.from_name('efficientnet-b7', num_classes=2,dropout_rate=0)
model=EfficientEnsemble_10fold(model_base,checkpahts)
...
with torch.no_grad():
    for i,datas in enumerate(testdataloader):
        ...
        for o_idx,output in enumerate(outputs):
            #앞의 경로 제외하고 파일명만 불러서 첫열에
            name=names[o_idx].replace('\\','/').split('/')[-1]
            writecsv(outputpath + args.result_name,
                     [name, output[0]])
